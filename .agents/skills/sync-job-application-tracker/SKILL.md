...
description: Initialize or idempotently reconcile Jariel's Google Sheet application tracker from the local SQLite ledger, preserving manual status edits and excluding discovery-only or skip-only jobs. Use for tracker setup, application row upserts, or response-status reconciliation.
---
