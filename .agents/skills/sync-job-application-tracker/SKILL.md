...
4. Application status is always projected from SQLite and must never be preserved as a competing manual authority. Preserve non-empty manual next action, follow-up date, and notes unless an explicitly authorized newer state wins.
...
4. Queue status, re-review date, and expiry are derived control-plane fields. Preserve manual notes; accept a manual next action only when it does not contradict the current SQLite lifecycle.
