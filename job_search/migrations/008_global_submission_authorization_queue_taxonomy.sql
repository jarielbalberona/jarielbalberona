...
ALTER TABLE review_queue RENAME TO review_queue_legacy_authorization_statuses;
...
CREATE TABLE review_queue (
  ...
  queue_status TEXT NOT NULL CHECK (
    queue_status IN (
      'AUTO_READY', 'SOURCE_RESTRICTED', 'POLICY_UNCLEAR', 'HOLD',
      'VIDEO_REQUIRED', 'READY_TO_RETRY', 'FORM_INACCESSIBLE', 'CLOSED'
    )
  ),
...
SELECT
  ...
  CASE queue_status
    WHEN 'MANUAL_APPLY' THEN 'POLICY_UNCLEAR'
    WHEN 'PREPARED' THEN 'HOLD'
    ELSE queue_status
  END,
  hold_review_reason, next_action, compensation, key_matches, material_gaps,
  prepared_screening_answers, cover_letter, cv_version, media_requirement,
  source_ats_policy, re_review_after, notes, created_at, updated_at
FROM review_queue_legacy_authorization_statuses;

DROP TABLE review_queue_legacy_authorization_statuses;
